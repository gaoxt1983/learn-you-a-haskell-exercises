...
power :: Int -> Int -> Int
power x y = if y == 1 then x else x * (power x (y - 1))
...
fib :: (Num a, Eq a) => a -> [a]
fib x = if x == 0 then [0] else if x == 1 then [1, 0] else (x1 + x2) : fibPre
    where
        fibPre = fib (x - 1)
        x1:x2:xs = fibPre
...
stepReverseSign a = (if num >= 0 then 1 else -1) * (abs(num) + step)
        where (num, step) = a
